...
> 版本：V3.8（2026-08-16，混合技能去个人化脱敏 + 移除 7 个纯个人技能引用，总计 250 项）
> 来源：Hermes 本地技能库 + Claude Code 技能目录（含 arkcli 工具族）+ OpenCode 技能目录 + Aion CLI
> 定位：本地技能仓库维护中（定规：先本地完善，后择机开源）——给任意支持 SKILL.md 的 Agent 直接装配
> 排除项：敏感技能（个人商业/家庭经营/职业背景类）+ 平台绑定技能（hermes/claude/aionui/openclaw专属）
...
技能开源仓库/
├── README.md                    # 本文件
├── CATALOG.md                   # 技能分类目录（250 项，逐项带描述，自动核验）
...
> 修订说明：V3.8 移除 7 个纯个人技能（grant-application-strategist/official-form-filling/reference-verification/academic-reference-verification/skill-import/docx-template-fill/paper2code）的引用；V3.7 起移除清单中不存在的技能名（如 academic-paper-skills、cnki-paper-download、taste-skill-official 等占位/幽灵条目），并消除重复归类（bootstrap/github 等只入一类）。

### 🎓 学术科研（31 项）
academic-figure, academic-paper-composer, academic-paper-composer-zh, academic-paper-review, academic-paper-reviewer, academic-paper-strategist, academic-paper-strategist-zh, academic-rewriter, aigc-check, arxiv, deep-research, drawio-scientific-figure, evidence-gate, gb7714-reference-format, github-deep-research, hermes-arxiv-agent, journal-adapt, latex-document-skill, literature-review, no-ai-slop, no-ai-slop-zh, office-academic-skill, paper-research, paper-to-patent, paperjury, research-paper-writing, research-writing-skill, scientific-toolkit-skill, simple-english, systematic-literature-review, systematic-literature-review-method
...
### 📦 生产力办公（22 项）
1password, airtable, data-analysis, docx, gog, google-workspace, jupyter-notebook, maps, nano-pdf, notion, ocr-and-documents, officecli, openhue, pdf, pdf-hybrid-reader, powerpoint, ppt-generation, report-to-ppt, slides, spreadsheet, teams-meeting-pipeline, xlsx
...
| 类型 | 技能 | 原因 |
|------|------|------|
| 敏感 | 涉及个人商业/家庭经营信息的技能 | 个人商业信息 |
| 敏感 | 涉及个人职业背景的技能 | 涉个人职业背景 |
| 平台绑定 | hermes-agent, hermes-windows, aionui-team-leadership, openclaw-ops 等 30 项 | 仅限特定平台运行 |

## 四、维护记录

- V3.8（2026-08-16）：混合技能去个人化脱敏（个人商业/家装专名→通用表述，称谓与用户反馈引用清理，11 个技能文件 + 3 份技能解读）；移除 7 个纯个人技能（grant-application-strategist/official-form-filling/reference-verification/academic-reference-verification/skill-import/docx-template-fill/paper2code）在 README/CATALOG 的引用，总计 257→250 项
...
- V3.6（2026-08-16）：本机语境型身份信息清理（院校/人名/申报细节→占位，14 文件）+ README 本机路径清理；收紧半身份证区域码脱敏示例
...
- V3.1（2026-08-07）：技能盘点入库 47 项（arkcli 工具族 24 + Letta 生态 10 + 独立工具 13），总计 226→273 项；OpenCode/Aion CLI 复核无新增；另有 2 项平台绑定不入库
...
- V1.0（2026-08-07）：5 项自研技能 + 学术方法论文档 + 实验工具箱
