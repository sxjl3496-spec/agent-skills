...
- `agent-slack auth test [--workspace]` — verify credentials (`auth.test`)
...
- `agent-slack auth add --workspace-url <url> [--token <tok> | --xoxc <c> --xoxd <d>]`
- `agent-slack auth set-default <workspace>`
- `agent-slack auth remove <workspace>`
...
- `agent-slack message get <target>`
 ...
 - Options:
   ...
   - `--ts. <ts>` (required when targeting a channel)
   - `--thread-ts. <ts>` (optional hint for thread permalinks)
...
- `agent-slack message list <target>`
 ...
 - Options:
   ...
   - `--thread-ts. <ts>` (switches to thread mode; fetches replies)
   - `--ts. <ts>` (resolve a message to its thread)
...
- `agent-slack message draft <target> [text]`
 ...
 - Options:
   ...
   - `--thread-ts. <ts>` (optional, channel mode only)
...
- `agent-slack message edit <target> <text>`
 ...
 - Options:
   - `--workspace <url>` (needed for channel _names_ across multiple workspaces)
   - `--ts. <ts>` (required for channel targets)
...
- `agent-slack channel list [--workspace] [--user | --all] [--limit] [--cursor]`
...
- `agent-slack channel new --name <name> [--private] [--workspace]`
- `agent-slack channel invite --channel <ch> --users "<list>" [--workspace]`
...
- `--channel <ch>` repeatable (`#name`, `name`, or id)
- `--user <@name|name|U...>`
- `--after YYYY-MM-DD`
- `--before YYYY-MM-DD`
...
- `agent-slack canvas get <url-or-id>`
 - Options:
...
- `agent-slack user list [--workspace] [--limit] [--cursor] [--include-bots]`
- `agent-slack user get <user> [--workspace]`
- `agent-slack user dm-open <user...> [--workspace]` — get DM or group DM channel ID for one or more users (max 8)
