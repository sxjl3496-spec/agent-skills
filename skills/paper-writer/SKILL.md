...
End-to-end research paper builder. **Single stage, full quality from the start** — there is no skeleton phase to enrich later. The agent (Codex) does the writing using its own tools (Codex web open/fetch, Codex web search, Write, Bash). This skill has no Python runtime; it is purely a procedure + reference playbooks + a LaTeX template.
...
| `references/01-bibliography-expansion.md` | grow `bibliography.bib` to 200+ real entries via Codex web open/fetch/Codex web search |
...
The full pass does not fit in a single turn. The bibliography is built across ~20+ small Codex web open/fetch/Codex web search batches; sections are drafted one per turn; figures are generated one at a time. **Read `references/00-incremental-execution.md` before starting** — it is the only execution mode that actually completes without losing work.
...
Plan 15–25 query angles for the topic. For each angle: Codex web search → pick candidates → Codex web open/fetch each candidate's abstract / arXiv API URL → extract canonical title/authors/year/venue/url → append a BibTeX entry to `$RUN/bibliography.bib`. **Every entry must originate from a URL fetched in this session.** Memory entries are forbidden.
...
- `output/literature-survey/<slug>/latest/bibliography.bib` → seed `$RUN/bibliography.bib` (still bring it up to 200+ in 3.1 with Codex web open/fetch).
