...
End-to-end experiment package builder. **Single stage, full quality from the start.** The agent (Codex) writes everything directly using its own tools (Write, Bash, Codex web open/fetch, …). This skill contains procedure + reference playbooks + figure-example scripts — no Python runtime, no LLM SDK.
