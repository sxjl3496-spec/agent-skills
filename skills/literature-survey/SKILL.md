...
End-to-end literature survey builder. **Single stage, full quality from the start.** The agent (Codex) does the entire build using its own tools (Codex web open/fetch, Codex web search, Write, Bash). This SKILL is procedure + reference playbooks + LaTeX template — no Python runtime, no LLM SDK.
...
| `references/01-bibliography-expansion.md` | grow `bibliography.bib` to 60+ real entries (100+ recommended) via Codex web open/fetch (no memory) |
...
Then plan **12–20** query angles, weighted by the posture. For each angle: Codex web search → triage → Codex web open/fetch each kept candidate's abstract URL → extract canonical title/authors/year/venue/url → append a BibTeX entry to `$RUN/bibliography.bib`. **Every entry must originate from a URL fetched in this session.** Memory entries forbidden.
