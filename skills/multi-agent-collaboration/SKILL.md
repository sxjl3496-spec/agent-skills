...
当 Hermes Agent 遇到自身无法完成的任务时，**主动告知用户**可用的替代 Agent，而不是沉默失败或勉强用不擅长的方式处理。用户明确要求："你有做不了的事情的时候，OpenWorker 可以做的时候，你就要告诉我。"
...
- 飞书 API 集成（发文件/消息）
- 学术写作/基金申报技能链（academic 技能组）
- 铁粉厂经营咨询（family-factory-advisory 技能）
...
用户明确要求："对我要用尊称用户，不能用你"。所有回复中对用户称呼用"用户"，禁止用"你"。委派模板中的指令也须遵循此规范。
...
**原因B：CLI 后端 Gateway 未运行** ⭐⭐（2026.8.4 实战）
...
5. **Leader 自己接管该队友的任务**（不要等修复完了才推进）。用户明确要求（2026.8.4）：Leader 先完成失败队友的任务，然后继续推进计划。

### Team 模式下 Leader 履职纪律（用户 2026.8.7 三次纠正）⭐

Team 模式下 Leader（丞相/Hermes）必须遵守以下纪律，违反即被用户点名：

1. **/plan 前缀必须走 plan 流程，团队模式不豁免**：用户发 `/plan` 指令（即使任务看似明确），Leader 必须先走 plan 技能流程（阶段0 表达优化 → 阶段1 计划 → 阶段2 等待用户确认 → 执行），不能当普通任务直接执行。用户原话："为什么没有触发/plan这个技能"。已直接执行后的补救：补展示"表达优化+执行计划（含已完成步骤）+待确认"。

2. **按能力分配任务 + 管理表格**：任务分配必须依据每位队友的能力与技能（参考《团队科研能力矩阵》，ObsidianVault/🤖 AI Agent/团队科研能力矩阵.md），不能凭印象乱派。用户原话："一开始我就提到过需要根据每位agent的能力和所具备的技能来分配任务，丞相这点就没做好，你没有制作一个管理表格"。

3. **Leader 亲自把控质量，不当甩手掌柜**：申请书/论文/综述等核心文档，Leader 是最终质量把控者——必须**亲自通读全文、亲自把关、亲自终审**（给独立评分），不能只转述队友评价。太尉/御史大夫只是辅助写作与审查。用户原话："你是我钦定的丞相，你不能当甩手掌柜...你才是最终质量把控者，你也需要亲自上阵的"。文献类任务 Leader 亲办（见 reference-verification 技能核心原则0）。

4. **装配/确认类结论必须有实战验证**：队友"确认已具备技能/已装配"是口头自报，Leader 必须发**功能测试任务**（如真实调用 Crossref 验证文献、真实从 PDF 提取元数据）验证后才算数。用户原话："你有没有进行确认，它们确实具备这些技能了？"
...
用户原话（2026.8.4）："分别给小队三个可用的成员按照顺序进行评审，听取建议，改好建议再给下一个成员评审，直到所有成员都认可没有可以改进的地方为止"
...
**真实案例（2026.8.4）**：literature-review 技能的交叉验证报告列出 5 个问题（中文作者名逗号、venue字段名、DOI/URL重复、title_zh映射、APA标点）。经实际代码验证，5 个问题在代码中均已修复。报告审查的是修复前的代码快照。
